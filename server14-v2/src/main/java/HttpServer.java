...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        if (file.isFile()) {
            ...
            response.setHeader("ETag", response.hashing());
            if (request.headers.containsKey("If-None-Match") && request.headers.get("If-None-Match").equals(response.headers.get("ETag"))) {
    ...
    private static final class Response {
        ...
        private String hashing() {
            String tmpString = Integer.toString(Arrays.hashCode(body));
            if (tmpString.length() < 5) {
                for (int i = 0; i < 5; i++) {
                    tmpString = tmpString.concat(tmpString);
                }
            }

            if (tmpString.length() > 64) {
                tmpString = tmpString.substring(0, 63);
            }
